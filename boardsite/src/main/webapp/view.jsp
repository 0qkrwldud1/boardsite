...
-->
	...
	<c:if test="${user_ID == user_id}">			
		<button class = "btn" type="button" onclick="chkDelete(<%=dto.getNum()%>, <%=nowpage%>)">Delete</button>
    ...
    <script>
    function chkDelete(num, nowpage) {
    	let r = confirm("삭제하시겠습니까?");
        if (r) {
        	location.href = "deleteOk.do?num=" + num +"&pageNum=" + nowpage;
        }
    }
    </script>
